...
function! floaterm#wrapper#fzf#(cmd, jobopts, config) abort
  ...
  if cmd !~ '--preview'
    if executable('bat')
      let cmd .= ' --preview ' . shellescape('( [[ -f {} ]] && (bat --style=plain --color=always --theme=TwoDark --pager=never {} || cat {})) || ([[ -d {} ]] && (eza --icons -T --color=always --group-directories-first {} || tree -C {})) || echo {} 2> /dev/null | head -200')
    else
