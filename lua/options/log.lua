...
local logfile = string.format("%s/%s.log", get_cache_dir(), "options")
...
function Log:init()
  ...
  structlog.configure(pvim_log)
  local logger = structlog.get_logger "options"

  -- Overwrite `vim.notify` to use the logger
  if options.log.override_notify then
